...
LAWIN_CLASSIFICATIONS_DIR = Utils.construct_path_from_project_root('data/final_classifications_lawin')


def run_topic_modelling(dir):
    TOPIC_MODELS_DIR = os.path.join(dir, 'topic_models')

    lda_topic_modeller = LDATopicModeller()

    # load classifications
    files = FolderIO.get_files(dir, False, '.csv')
    ...
    for file in files:
        ...
        preprocessors = [
            ...
            PreProcessing.RemovePunctuationFromWords(remove_all=True),
            ...
            PreProcessing.RemoveNonAlphabet(),
        ...
        topic_model_file = open(os.path.join(TOPIC_MODELS_DIR, category+".pickle"), "wb")
        pickle.dump(topic_model, topic_model_file)
        topic_model_file.close()

        topic_model_string_file = open(os.path.join(TOPIC_MODELS_DIR, category+".txt"), "w", encoding='utf-8')
        topic_model_string_file.write(topic_model_string)
        topic_model_string_file.close()



# run_topic_modelling(YOLANDA_CLASSIFICATIONS_DIR)
run_topic_modelling(LAWIN_CLASSIFICATIONS_DIR)